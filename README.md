...
| **`ragy init`** | `[<dataset_id>]` | カレントディレクトリをプロジェクトとして初期化。Difyに同名データセットがなければAPIで自動新規作成（既存IDでの紐付けも可能）。`.rag-project`および`docs`シンボリックリンクを自動展開します。 |
| **`ragy sync`** | - | カレントディレクトリに紐づくローカル文書（画像含む）を、Difyの対応データセットへ即時手動同期。 |
| **`ragy doctor`** | - | **システム環境診断**: CLIツール、Pythonライブラリ、ネットワークポート競合、環境変数を一撃でチェック・レポート。 |
...
# プロジェクトを自動初期化（Dify上に同名データセットが自動新規作成され、.rag-projectが生成されます）
ragy init

# （既存のDifyデータセットと直接紐付けたい場合は、引数にデータセットIDを指定可能）
